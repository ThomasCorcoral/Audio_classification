...
class Base(nn.Module):
    def __init__(
        ...
        if not dimension_reduction_rate < 1.0:
            self.dimension_reduction_rate = 1.0
        else:
    ...
    def calculate_scatter_maxpool(self, score, feature, out_len):
        bs, _, feat_dim = feature.size()
        cumsum = torch.cumsum(score, dim=1)

        # /!\ float pb
        cumsum[cumsum % 1 < 1e-2] -= 1e-2  # If the sum is equal to an integer
    ...
    def locate_first_and_last_position(self, mask):
        _, orig_len, target_len = mask.size()
    ...
    def score_norm(self, score, total_length):
        ...
        # Remove zero pad at the end
        if torch.sum(dims_need_norm) > 0:
            sum_score = torch.sum(score, dim=(1, 2), keepdim=True)
            distance_with_target_length = (total_length - sum_score)[:, 0, 0]
            axis = torch.logical_and(score < RESCALE_INTERVEL_MAX, score > RESCALE_INTERVEL_MIN)
